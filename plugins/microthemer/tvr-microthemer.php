		...
		class tvr_microthemer_admin {
			...
			function microthemer_ui_page() {
				...
				if ( isset($_GET['page']) and $_GET['page'] == $this->microthemeruipage ) {
					...
					if (isset($_POST['tvr_ui_validate_submit'])) {
						...
						$params = 'email='.rawurlencode($_POST['tvr_preferences']['buyer_email'])
                        ...
                        if ( strpos($responseString, '{') !== false ){
	                        ...
	                        $validation = $response['unlock'];
                        }

                        // and old response format
                        else {
	                        $validation = $responseString && strlen($responseString) < 2;
                        }

						//$this->show_me = 'strlen: ' . strlen($responseString) . '<br />' . $responseString;
						...
						if ($validation) {
							...
							// if no response, check for http issue
                            ...
                            if (!empty($response['message'])){
	                            ...
	                            $title = 'Unlock failed - ' . $response['message'];
                                ...
                                switch ($response['message']) {
			                        ...
			                        case "invalid credentials":
                                        ...
                                        <p>If you purchased Microthemer from <b>CodeCanyon</b>, please send us a "Validate 
                                        my email" message via the contact form on the right hand side of 
                                        <a target="_blank" href="http://codecanyon.net/user/themeover">this page</a>. 
                                        You will need to ensure that you are logged in to CodeCanyon.</p>';
			                            break;
	                                ...
	                                case "connection error":
                                        ...
                                        HTTP response code: '.$response_code.'</p>
                                    
                                        <p>The connection to Themeover\'s server may have failed due to an 
                                        intermittent network error. <span class="link show-dialog" 
			...
			*/
			...
			function icon_control(
				...
				$imp_key = '';
				if ($level == 'tab-input' or $level == 'subgroup'){
